...
const TYPE_INFO = {
  pc_standard: { label: "Standard PC", icon: Monitor, color: "text-primary" },
  pc_vip: { label: "VIP PC", icon: Monitor, color: "text-secondary" },
  console: { label: "Console", icon: Gamepad, color: "text-accent" },
  room: { label: "Private Room", icon: DoorOpen, color: "text-warning" },
} as const;
    ...
    <div className="container py-12 md:py-16">
      <div className="max-w-2xl mb-10">
        <h1 className="font-display text-4xl md:text-5xl mb-3">
          Stations & <span className="text-gradient-neon">Rooms</span>
        </h1>
        <p className="text-muted-foreground">
          Pick your weapon. Standard PCs for casual sessions, VIP rigs for tournaments,
          consoles for couch co-op, and private rooms for squad nights.
        </p>
      </div>
        ...
        return (
          <section key={type} className="mb-12">
            <div className="flex items-center gap-3 mb-5">
              ...
              <Badge variant="outline" className="ml-auto">{items.length} available</Badge>
            </div>
              ...
              {items.map((s) => (
                <Card key={s.id} className="p-5 bg-card/60 border-border/60 hover:border-primary/50 transition-all hover:-translate-y-0.5">
                  <div className="flex items-start justify-between mb-3">
                    ...
                    <span className="text-secondary font-bold">${s.hourly_rate}/hr</span>
                  </div>
                  {s.description && <p className="text-sm text-muted-foreground mb-3">{s.description}</p>}
                  <div className="flex items-center justify-between mt-4">
                    <span className="text-xs text-muted-foreground">Seats: {s.capacity}</span>
                    <Button asChild size="sm" variant="outline">
                      <Link to={`/dashboard/book?station=${s.id}`}>Book</Link>
                    </Button>
                  </div>
                </Card>
              ))}
